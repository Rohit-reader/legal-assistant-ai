...
def extract_text_with_format(pdf_path: str) -> List[Dict]:
...
def identify_bns_sections(text_blocks: List[Dict]) -> pd.DataFrame:
    """Identify BNS sections from the extracted text blocks."""
    sections = []
    ...
    for i, block in enumerate(text_blocks):
        ...
        # Look for section patterns (e.g., "1. Text" or "Section 1: Text")
        section_match = re.match(r'^(\d+)\.\s+(.+?)(?=\s*$|\s+\d+\.)', text)
        
        if section_match and (block['font_size'] > 10 or block['bold']):
...
def main():
    ...
    output_path = project_root / "data" / "raw" / "bns_sections_detailed.csv"
    
    print(f"Extracting BNS sections from {pdf_path}...")
    
    # Extract text with detailed formatting
    print("Extracting text with formatting information...")
    text_blocks = extract_text_with_format(pdf_path)
    print(f"Extracted {len(text_blocks)} text blocks.")
    
    # Identify sections
    print("Identifying sections...")
    df = identify_bns_sections(text_blocks)
    ...
    if not df.empty:
        ...
        # Save sample of text blocks for debugging
        sample_blocks = pd.DataFrame(text_blocks[:50])  # First 50 blocks for analysis
        sample_path = project_root / "data" / "raw" / "sample_text_blocks.csv"
        sample_blocks.to_csv(sample_path, index=False)
        print(f"\nSaved sample of {len(sample_blocks)} text blocks to {sample_path} for analysis.")
